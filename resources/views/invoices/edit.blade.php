...
@section('subtitle', 'Modify draft invoice')
...
@section('content')
  @php
    ...
    $existingItems = $invoice->items->map(function ($it) {
      $packageId = $it->package_id ? (string) $it->package_id : '';
      $serviceId = $it->service_id ? (string) $it->service_id : '';
      $selected = $packageId ? 'p-' . $packageId : ($serviceId ? 's-' . $serviceId : '');
      return [
        'key' => $it->id,
        'selected' => $selected,
    ...
    $serviceIds = $invoice->items->pluck('service_id')->filter()->unique();
    $extraServices = \App\Models\Service::whereIn('id', $serviceIds)->get();
    $servicesForEdit = $services->merge($extraServices)->unique('id')->sortBy('name')->values();
    $servicesForView = $servicesForEdit->map(fn ($s) => ['id' => (string) $s->id, 'name' => $s->name, 'taxable' => (float) $s->taxable_amount, 'non_taxable' => (float) $s->non_taxable_amount])->values()->toArray();
    // Ensure every package on the invoice is in the list (by id + name), so the dropdown can show the selected package
    $packagesArray = isset($packages) ? $packages->values()->toArray() : [];
    $packagesById = collect($packagesArray)->keyBy('id')->all();
    foreach ($invoice->items as $it) {
      if ($it->package_id && $it->relationLoaded('package') && $it->package) {
        $p = $it->package;
        $id = (int) $p->id;
        if (! isset($packagesById[$id])) {
          $packagesById[$id] = [
            'id' => $id,
            'name' => $p->name,
    ...
    $packagesForView = array_values($packagesById);
    $initialDiscount = (float) ($invoice->discount_amount ?? 0);
    $initialRounding = (float) ($invoice->rounding_adjustment ?? 0);
  @endphp

  <form method="POST" action="{{ route('invoices.update', $invoice) }}"
        x-data='invoiceBuilder({{ $vatRate }}, @json($servicesForView), @json($packagesForView), @json($existingItems), {{ $initialDiscount }}, {{ $initialRounding }})'
        x-init="
          $nextTick(() => $nextTick(() => {
            items.forEach(row => {
              if (row.package_id) row.selected = 'p-' + String(row.package_id);
              else if (row.service_id) row.selected = 's-' + String(row.service_id);
            });
            setTimeout(() => {
              const selects = $el.querySelectorAll('select[data-item-select]');
              selects.forEach((sel, idx) => {
                if (items[idx] && items[idx].selected) sel.value = items[idx].selected;
              });
            }, 80);
          }));
        "
      ...
      <div class="card p-5">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input name="invoice_date" type="date" class="input mt-1 @error('invoice_date') border-rose-300 ring-1 ring-rose-200 @enderror" value="{{ old('invoice_date', $invoice->invoice_date?->toDateString()) }}" required>
            @error('invoice_date')
              <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
            @enderror
          </div>
          <div x-data="{ setDue(val) { $refs.dueInput.value = val; } }">
            ...
            <input name="due_date" type="date" class="input mt-1 @error('due_date') border-rose-300 ring-1 ring-rose-200 @enderror" value="{{ old('due_date', $invoice->due_date?->toDateString()) }}" x-ref="dueInput">
            <div class="mt-1 flex gap-2">
              <button type="button" class="text-xs text-brand-600 hover:underline" @click="setDue('{{ ($invoice->invoice_date ?? now())->copy()->addDays(7)->toDateString() }}')">Net 7</button>
              <button type="button" class="text-xs text-brand-600 hover:underline" @click="setDue('{{ ($invoice->invoice_date ?? now())->copy()->addDays(15)->toDateString() }}')">Net 15</button>
              <button type="button" class="text-xs text-brand-600 hover:underline" @click="setDue('{{ ($invoice->invoice_date ?? now())->copy()->addDays(30)->toDateString() }}')">Net 30</button>
            </div>
            @error('due_date')
              <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
            @enderror
        ...
        <div class="mt-4">
          ...
          <textarea name="notes" class="input mt-1" rows="3">{{ old('notes', $invoice->notes) }}</textarea>
        </div>
      </div>
            ...
            <tbody>
              <template x-for="(row, idx) in items" :key="row.key">
                <tr class="hover:bg-slate-50 transition">
                  <td class="table-td min-w-[280px] align-middle">
                    ...
                    <select class="input w-full" data-item-select x-model="row.selected"
                      ...
                      <optgroup label="Services">
                        <template x-for="s in services" :key="'s-'+s.id">
                          <option :value="'s-'+String(s.id)" x-text="s.name"></option>
                        </template>
                      </optgroup>
                      <optgroup label="Packages">
                        <template x-for="p in packages" :key="'p-'+p.id">
                          <option :value="'p-'+String(p.id)" x-text="p.name"></option>
                        </template>
                      </optgroup>
                    </select>
                  </td>
                  <td class="table-td w-[110px]">
                    <input class="input" type="number" min="1" step="1" x-model.number="row.quantity" :name="'items['+idx+'][quantity]'" required>
                  </td>
                  <td class="table-td"><span class="font-semibold" x-text="money(lineNonTaxable(row))"></span></td>
                  <td class="table-td"><span class="font-semibold" x-text="money(lineTaxable(row))"></span></td>
                  <td class="table-td"><span class="font-semibold" x-text="money(lineVat(row))"></span></td>
                  <td class="table-td"><span class="font-semibold" x-text="money(lineTotal(row))"></span></td>
    ...
    <div class="space-y-4">
      <div class="card p-5 sticky top-24">
        ...
        <button class="btn-primary w-full mt-5 flex items-center justify-center gap-2" type="submit"
          ...
          <span x-text="submitting ? 'Updating...' : 'Update Invoice'"></span>
        </button>
        <a class="btn-ghost w-full mt-2 text-center" href="{{ route('invoices.show', $invoice) }}">Cancel</a>
      </div>
    </div>
  </form>
  ...
  <script>
    function invoiceBuilder(vatRate, services, packages, initialItems = null, initialDiscount = 0, initialRounding = 0) {
      ...
      return {
        ...
        serviceById(id) { return this.services.find(s => String(s.id) === String(id)); },
        packageById(id) { return this.packages.find(p => Number(p.id) === Number(id)); },
        getSelectedName(row) {
          if (row.package_id) { const p = this.packageById(row.package_id); return p ? p.name : null; }
          if (row.service_id) { const s = this.serviceById(row.service_id); return s ? s.name : null; }
          if (row.selected && row.selected.startsWith('p-')) { const p = this.packageById(row.selected.slice(2)); return p ? p.name : null; }
          if (row.selected && row.selected.startsWith('s-')) { const s = this.serviceById(row.selected.slice(2)); return s ? s.name : null; }
          return null;
        },
        lineTaxable(row) {
          const qty = Number(row.quantity || 0);
          if (row.selected && row.selected.startsWith('p-')) { const p = this.packageById(row.selected.slice(2)); return p ? Number(p.taxable) * qty : 0; }
          const s = this.serviceById(row.service_id); return s ? Number(s.taxable) * qty : 0;
        },
        lineNonTaxable(row) {
          const qty = Number(row.quantity || 0);
          if (row.selected && row.selected.startsWith('p-')) { const p = this.packageById(row.selected.slice(2)); return p ? Number(p.non_taxable) * qty : 0; }
        ...
        money(v) { const n = Math.round(Number(v || 0) * 100) / 100; return n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' AED'; },
      };
    }
  </script>
@endsection
