...
@section('subtitle', 'Update package services, discount and rounding')

@section('content')
  @php
    $packageItems = $package->items->map(fn ($it) => [
      'service_id' => (string) $it->service_id,
      'quantity' => $it->quantity,
      'taxable_amount' => $it->taxable_amount !== null ? (float) $it->taxable_amount : '',
      'non_taxable_amount' => $it->non_taxable_amount !== null ? (float) $it->non_taxable_amount : '',
    ])->values()->all();
    if (empty($packageItems)) {
      $packageItems = [['service_id' => '', 'quantity' => 1, 'taxable_amount' => '', 'non_taxable_amount' => '']];
    }
    ...
    $packageItemsJson = json_encode($packageItems);
  @endphp
  <div x-data='packageForm({!! $servicesJson !!}, {!! $packageItemsJson !!})'>
    <form action="{{ route('packages.update', $package) }}" method="POST">
      ...
      <div class="card p-5 space-y-5">
        <div>
          ...
          <input class="input mt-1 w-full" type="text" name="name" value="{{ old('name', $package->name) }}" required>
        </div>
        <div>
          <label class="label">Description</label>
          <textarea class="input mt-1 w-full" name="description" rows="2">{{ old('description', $package->description) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div class="space-y-3">
            ...
            <div class="flex flex-wrap gap-3 items-center">
              <label class="inline-flex items-center gap-2">
                <input type="radio" name="discount_type" value="" x-model="discountType"> None
              </label>
              <label class="inline-flex items-center gap-2">
            ...
            <div x-show="discountType" class="flex items-center gap-2">
              <input type="number" name="discount_value" step="0.01" min="0" class="input w-32" x-model.number="discountValue">
          ...
          <div class="space-y-3">
            ...
            <select name="rounding_rule" class="input w-full" x-model="roundingRule">
              ...
              <option value="nearest_5" {{ old('rounding_rule', $package->rounding_rule) === 'nearest_5' ? 'selected' : '' }}>Nearest 5</option>
              <option value="nearest_10" {{ old('rounding_rule', $package->rounding_rule) === 'nearest_10' ? 'selected' : '' }}>Nearest 10</option>
              <option value="nearest_50" {{ old('rounding_rule', $package->rounding_rule) === 'nearest_50' ? 'selected' : '' }}>Nearest 50</option>
              <option value="custom" {{ old('rounding_rule', $package->rounding_rule) === 'custom' ? 'selected' : '' }}>Custom target (e.g. 670)</option>
            </select>
            <div x-show="roundingRule === 'custom'" class="mt-2">
              <input type="number" name="rounding_target" step="0.01" min="0" class="input w-32" x-model.number="roundingTarget">
            </div>
          </div>
        </div>
  ...
  <script>
    document.addEventListener('alpine:init', () => {
      Alpine.data('packageForm', (servicesList, initialItems) => ({
        ...
        discountType: '{{ old('discount_type', $package->discount_type ?? '') }}',
        discountValue: {{ old('discount_value', $package->discount_value ?? 0) }},
        roundingRule: '{{ old('rounding_rule', $package->rounding_rule ?? '') }}',
        roundingTarget: {{ old('rounding_target', $package->rounding_target ?? 0) }},
        ...
        onServiceChange(i, serviceId) {
          ...
          if (s && (this.items[i].taxable_amount === '' || this.items[i].taxable_amount === null)) {
            this.items[i].taxable_amount = s.taxable_amount;
            this.items[i].non_taxable_amount = s.non_taxable_amount;
          }
        },
        lineTaxable(row) {
